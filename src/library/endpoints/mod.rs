...
macro_rules! decrypt {
    ($text:expr) => {
        |text: String| -> Result<String, Box<dyn std::error::Error>> {
            ...
            Ok(String::from_utf8(arthur_hugon_tools::reexports::decode(
                &arthur_hugon_tools::security::private_decrypt(
                    &key,
                    serde_json::from_str::<arthur_hugon_tools::security::Tomb>(
                        String::from_utf8(arthur_hugon_tools::reexports::decode(&text)?)?.as_str(),
                    )?,
                )?
                .value,
            )?)
...
macro_rules! response {
    ($code:ident, $json:expr) => {
        || -> Result<HttpResponse, Box<dyn std::error::Error>> {
            let key = arthur_hugon_tools::reexports::rsa::Rsa::public_key_from_pem(
            ...
            let response =
                arthur_hugon_tools::reexports::encode(&serde_json::to_string(&response).unwrap());
